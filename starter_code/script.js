window.onload = function() {
  ...
  document.onkeydown = function(e) {
    if (e.keyCode == 37 && (car.x >= 75 && car.x <= 190)) {
      car.moveLeft();
    }
    if (e.keyCode == 39 && (car.x >= 75 && car.x <= 190)) {
      car.moveRight();
    }
    if (e.keyCode == 38 && (car.y >= 0 && car.y <= 170)) {
      car.moveUp();
    }
    if (e.keyCode == 40 && (car.y >= 0 && car.y <= 170)) {
      car.moveDown();
    }
    startGame();
  }
  ...
  function paintCar() {  
    ctx.drawImage(img, car.x, car.y, 30, 30);
  }
