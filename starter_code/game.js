...
class Game {
    ...
    startGame() {
        document.querySelector('.crash').classList.remove('crashreal')
    ...
    createObst() {
        if (this.counter % 300 === 0) {
            this.obstaclesArr.push(new Obst(randomInt(50, 220), 40, randomInt(50, 220), 0));
        }
    ...
    obstacleCal() {
        this.obstaclesArr.forEach(obstacle => {
            if (this.car.carPosX < obstacle.posX + obstacle.width &&
                ...
                document.querySelector('.crash').classList.add('crashreal')
