...
/* objetos
 ...
 Pista: image width, height, posX, posY, context, canvas, drawField, moveField;

 Obstaculo: width, height, posX, posY, context, canvas, color, drawObstacle, moveObstacle, collisionVerification;
...
class Object {
  ...
  crashWith(obstacle) {
    return !(this.top() > obstacle.bottom() || this.left() > obstacle.right() || this.bottom() < obstacle.top() || this.right() < obstacle.left());
    // return !(this.bottom() < obstacle.top() || this.top() > obstacle.bottom() || this.right() < obstacle.left() || this.left() > obstacle.rigth());
  }
}
  ...
  move(keyCode, speed) {
    switch (keyCode) {
      case 37:
        ...
        break;
      case 39:
        if (this.posX > this.canvas.width - 125) return;
        this.posX += speed;
        break;
      default:
